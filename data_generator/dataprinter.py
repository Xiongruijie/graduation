...
def read_input_file(file_location="../Fieldsolver2d_hybrid_to_improve/input/input_0.data"):
    ...
    return dielectric, ConductorList
...
def print_img(ConductorList):
    ...
    # todo 归一化 画到图像上
    plt_list[:, 0] = (plt_list[:, 0] - np.min(plt_list[:, 0])) / (np.max(plt_list[:, 0]) - np.min(plt_list[:, 0]))
...
if __name__ == '__main__':
    dielectric, ConductorList = read_input_file()
